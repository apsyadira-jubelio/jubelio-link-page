...
const Link = {
	list: async (): Promise<LinkType[]> => {
		const links = await dbSystem.query(`
			select count(e.link_id) as total_click, l.*
			from links l 
			left join events e on e.link_id = l.id
			group by l.id order by position
		`);
	...
	reorder: async (id: number, newIndex: number, oldIndex: number): Promise<LinkType[]> => {
		const queryResult = await dbSystem.query(`
			select count(e.link_id) as total_click, l.*
			from links l 
			left join events e on e.link_id = l.id
			group by l.id order by position
		...
		const linkRows: LinkType[] = queryResult;
