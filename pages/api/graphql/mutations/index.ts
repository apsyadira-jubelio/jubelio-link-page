...
export const Mutation = {
	login: async (_: any, { password }: MutationLoginArgs, _context: any) => {
	...
	verify: async (_: any, params: { jwt: any }, _context: any) => {
	...
	updateLinkById: async (
		_: any,
		params: { label: string; content: string; id: number; position: number; type: string },
		{ isAuthorized }: { isAuthorized: boolean }
	) => {
		if (!isAuthorized) throw Error("Must be authorized to use the Dashboard.");
	...
	deleteLinkById: async (_: any, { id }: { id: number }, { isAuthorized }: { isAuthorized: boolean }) => {
		if (!isAuthorized) throw Error("Must be authorized to use the Dashboard.");
		Link.deleteById(id);
		return;
	},
