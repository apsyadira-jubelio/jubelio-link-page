		...
		<Dashboard>
			...
			<div className="flex flex-row items-center justify-between w-full mb-8 h1-row">
				...
				<Link href="/dashboard/link/create" passHref>
					<button className="flex flex-row items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg cursor-pointer hover:bg-indigo-700">
						<svg
			...
			{!reorder[1].loading ? (
								...
								{(links ?? []).map((link, i) => (
									<Draggable draggableId={(link?.id ?? 0).toString()} key={link?.id} index={i}>
										{(provided: any) => (
											...
											>
												<div
													className="w-full max-w-4xl p-6 mb-4 overflow-hidden bg-white shadow rounded-xl"
													style={{ borderLeft: "solid 12px rgba(0,0,0,.15)" }}
												>
													{link?.label && <div className="mb-2 text-xl font-medium">{link?.label}</div>}
													<div className="flex flex-row items-center justify-start space-x-4">
														<span className="capitalize">{link?.type}</span>
														<span>|</span>
														<a className="hover:underline hover:text-indigo-600" href={link?.content ?? "#"}>
				...
				<p className="text-lg text-gray-700">Loading links...</p>
			)}
		</Dashboard>
	);
};
